// Vendored from https://github.com/vitaut/zmij at a24bf7fce0f4fdee60c0153529b74bb537552b7b by tools/vendor-zmij.sh.
// Do not edit: change the script and run it again. What differs from upstream, and why, is
// described there. The licence is beside this file.
//
...
#ifndef SERPENT_ZMIJ_USE_SIMD
#  define SERPENT_ZMIJ_USE_SIMD 1
#endif

#ifdef SERPENT_ZMIJ_USE_NEON
// Use the provided definition.
#elif defined(__ARM_NEON) || defined(_M_ARM64)
#  define SERPENT_ZMIJ_USE_NEON SERPENT_ZMIJ_USE_SIMD
#else
#  define SERPENT_ZMIJ_USE_NEON 0
#endif
#if SERPENT_ZMIJ_USE_NEON
#  include <arm_neon.h>
#endif

#ifdef SERPENT_ZMIJ_USE_SSE
// Use the provided definition.
#elif defined(__SSE2__)
#  define SERPENT_ZMIJ_USE_SSE SERPENT_ZMIJ_USE_SIMD
#elif defined(_M_AMD64) || (defined(_M_IX86_FP) && _M_IX86_FP == 2)
#  define SERPENT_ZMIJ_USE_SSE SERPENT_ZMIJ_USE_SIMD
#else
#  define SERPENT_ZMIJ_USE_SSE 0
#endif
#if SERPENT_ZMIJ_USE_SSE
#  include <immintrin.h>
#endif

#ifdef SERPENT_ZMIJ_USE_SSE4_1
// Use the provided definition.
static_assert(!SERPENT_ZMIJ_USE_SSE4_1 || SERPENT_ZMIJ_USE_SSE);
#elif defined(__SSE4_1__) || defined(__AVX__)
// On MSVC there's no way to check for SSE4.1 specifically so check __AVX__.
#  define SERPENT_ZMIJ_USE_SSE4_1 SERPENT_ZMIJ_USE_SSE
#else
#  define SERPENT_ZMIJ_USE_SSE4_1 0
#endif

#ifdef __aarch64__
#  define SERPENT_ZMIJ_AARCH64 1
#else
#  define SERPENT_ZMIJ_AARCH64 0
#endif

#ifdef __x86_64__
#  define SERPENT_ZMIJ_X86_64 1
#else
#  define SERPENT_ZMIJ_X86_64 0
#endif

#ifdef __clang__
#  define SERPENT_ZMIJ_CLANG 1
#else
#  define SERPENT_ZMIJ_CLANG 0
#endif

#ifdef _MSC_VER
#  define SERPENT_ZMIJ_MSC_VER _MSC_VER
#else
#  define SERPENT_ZMIJ_MSC_VER 0
#endif

#if defined(__has_builtin) && !defined(SERPENT_ZMIJ_NO_BUILTINS)
#  define SERPENT_ZMIJ_HAS_BUILTIN(x) __has_builtin(x)
#else
#  define SERPENT_ZMIJ_HAS_BUILTIN(x) 0
#endif
#ifdef __has_attribute
#  define SERPENT_ZMIJ_HAS_ATTRIBUTE(x) __has_attribute(x)
#else
#  define SERPENT_ZMIJ_HAS_ATTRIBUTE(x) 0
#endif
#ifdef __has_cpp_attribute
#  define SERPENT_ZMIJ_HAS_CPP_ATTRIBUTE(x) __has_cpp_attribute(x)
#else
#  define SERPENT_ZMIJ_HAS_CPP_ATTRIBUTE(x) 0
#endif

#if SERPENT_ZMIJ_HAS_CPP_ATTRIBUTE(likely) && SERPENT_ZMIJ_HAS_CPP_ATTRIBUTE(unlikely)
#  define SERPENT_ZMIJ_UNLIKELY unlikely
#else
#  define SERPENT_ZMIJ_UNLIKELY
#endif

#if SERPENT_ZMIJ_HAS_CPP_ATTRIBUTE(maybe_unused)
#  define SERPENT_ZMIJ_MAYBE_UNUSED maybe_unused
#else
#  define SERPENT_ZMIJ_MAYBE_UNUSED
#endif

#ifdef SERPENT_ZMIJ_OPTIMIZE_SIZE
// Use the provided definition.
#elif defined(__OPTIMIZE_SIZE__)
#  define SERPENT_ZMIJ_OPTIMIZE_SIZE 1
#else
#  define SERPENT_ZMIJ_OPTIMIZE_SIZE 0
#endif
#ifndef SERPENT_ZMIJ_USE_EXP_STRING_TABLE
#  define SERPENT_ZMIJ_USE_EXP_STRING_TABLE SERPENT_ZMIJ_OPTIMIZE_SIZE == 0
#endif

#if SERPENT_ZMIJ_HAS_ATTRIBUTE(always_inline) && !SERPENT_ZMIJ_OPTIMIZE_SIZE
#  define SERPENT_ZMIJ_INLINE __attribute__((always_inline)) inline
#elif SERPENT_ZMIJ_MSC_VER
#  define SERPENT_ZMIJ_INLINE __forceinline
#else
#  define SERPENT_ZMIJ_INLINE inline
#endif

#ifdef __GNUC__
#  define SERPENT_ZMIJ_ASM(x) asm x
#else
#  define SERPENT_ZMIJ_ASM(x)
#endif

// Declares struct members that must live in memory on ARM64 but are encoded as
// immediates in the x64 assembly.
#ifdef SERPENT_ZMIJ_CONST_DECL
// Use the provided definition.
#elif SERPENT_ZMIJ_AARCH64
#  define SERPENT_ZMIJ_CONST_DECL
#else
#  define SERPENT_ZMIJ_CONST_DECL static constexpr
#endif
...
#if SERPENT_ZMIJ_HAS_BUILTIN(__builtin_bswap64)
...
inline auto clz(uint64_t x) noexcept -> int {
  assert(x != 0);
#if SERPENT_ZMIJ_HAS_BUILTIN(__builtin_clzll)
...
inline auto ctz(uint64_t x) noexcept -> int {
  assert(x != 0);
#if SERPENT_ZMIJ_HAS_BUILTIN(__builtin_ctzll)
...
SERPENT_ZMIJ_INLINE auto select(uint64_t condition, int64_t true_value,
                        int64_t false_value) -> int64_t {
  // Clang can figure it out on its own.
  if (!SERPENT_ZMIJ_X86_64 || SERPENT_ZMIJ_CLANG) return condition ? true_value : false_value;
  SERPENT_ZMIJ_ASM(
      volatile("test %2, %2\n\t"
               "cmovne %1, %0\n\t" :  //
...
using serpent::external::zmij::detail::compute_dec_exp;
using serpent::external::zmij::detail::compute_exp_shift;
using serpent::external::zmij::detail::float_traits;
using serpent::external::zmij::detail::umul128;
using serpent::external::zmij::detail::umul192_hi128;
using serpent::external::zmij::detail::uint128;
using serpent::external::zmij::detail::uint128_t;

#if SERPENT_ZMIJ_USE_INT128 && defined(__APPLE__)
...
SERPENT_ZMIJ_INLINE auto div10(uint64_t x) noexcept -> uint64_t {
  assert(x <= (1ull << 62));
  // ceil(2**64 / 10) computed as (1 << 63) / 5 + 1 to avoid int128.
  constexpr uint64_t div10_sig64 = (1ull << 63) / 5 + 1;
  return SERPENT_ZMIJ_USE_INT128 ? umul128_hi64(x, div10_sig64) : x / 10;
}
...
struct pow10_significand_table {
  static constexpr bool compress =
      SERPENT_ZMIJ_OPTIMIZE_SIZE != 0 || SERPENT_ZMIJ_USE_CONSTEXPR == 0;
  static constexpr bool split_tables = !compress && SERPENT_ZMIJ_AARCH64 != 0;
  static constexpr int num_pow10s = 649;
  uint64_t data[compress ? 1 : num_pow10s * 2] = {};

  static SERPENT_ZMIJ_CONSTEXPR auto make() -> pow10_significand_table {
    pow10_significand_table t;
    for (int i = 0; i < num_pow10s && !compress; ++i) {
      uint128 result = serpent::external::zmij::detail::compute_pow10(i - 307);
      if (split_tables) {
  ...
  SERPENT_ZMIJ_CONSTEXPR20 auto operator[](int dec_exp) const noexcept -> uint128 {
    constexpr int dec_exp_min = -307;
    int i = dec_exp - dec_exp_min;
    if (compress) return serpent::external::zmij::detail::compute_pow10(dec_exp);
    if (!split_tables) {
    ...
    if (!serpent::external::zmij::detail::is_constant_evaluated()) SERPENT_ZMIJ_ASM(("" : "+r"(p)));
    return {p[~dec_exp], p[~dec_exp + num_pow10s]};
  }
};

struct exp_shift_table {
  static constexpr bool enable = SERPENT_ZMIJ_OPTIMIZE_SIZE == 0 && SERPENT_ZMIJ_USE_CONSTEXPR;
  ...
  static SERPENT_ZMIJ_CONSTEXPR auto make() -> exp_shift_table {
    exp_shift_table t;
...
struct exp_string_table {
  static constexpr bool enable =
      SERPENT_ZMIJ_USE_EXP_STRING_TABLE && SERPENT_ZMIJ_USE_CONSTEXPR;
  ...
  static SERPENT_ZMIJ_CONSTEXPR auto make() -> exp_string_table {
    exp_string_table t;
...
struct exp_float_shuffle_table {
  static constexpr bool enable =
      (SERPENT_ZMIJ_USE_SSE4_1 || SERPENT_ZMIJ_USE_NEON) && exp_string_table::enable;
  ...
  SERPENT_ZMIJ_CONSTEXPR auto get_entry(int num_digits, bool has_last_digit,
  ...
  static SERPENT_ZMIJ_CONSTEXPR auto make() -> exp_float_shuffle_table {
...
constexpr auto fixed_entry_align() noexcept -> int {
  ...
  // than `umaddl`.
  return SERPENT_ZMIJ_USE_SSE4_1 ? 64 : SERPENT_ZMIJ_AARCH64 && !SERPENT_ZMIJ_OPTIMIZE_SIZE ? 32 : 1;
}
...
struct fixed_layout_table {
  static constexpr bool enable = SERPENT_ZMIJ_USE_CONSTEXPR != 0;
  using traits = float_traits<double>;
  ...
  static SERPENT_ZMIJ_CONSTEXPR auto compute(int dec_exp) noexcept -> entry {
    entry e = {};
  ...
  static SERPENT_ZMIJ_CONSTEXPR auto make() -> fixed_layout_table {
  ...
  SERPENT_ZMIJ_CONSTEXPR auto get(int dec_exp, entry& scratch) const noexcept
      -> const entry& {
...
SERPENT_ZMIJ_INLINE auto write2(char* out, char a, char b) noexcept -> char* {
...
struct data {
  ...
  SERPENT_ZMIJ_CONST_DECL uint64_t threshold = 1e15;
  // +6 is needed for boundary cases found by verify.py.
  SERPENT_ZMIJ_CONST_DECL uint64_t biased_half = (uint64_t(1) << 63) + 6;

#if SERPENT_ZMIJ_USE_NEON
  static constexpr int32_t neg10k = 0x10000 - 10000;

  using int32x4 =
      typename std::conditional<SERPENT_ZMIJ_MSC_VER != 0, int32_t[4], int32x4_t>::type;
  using int16x8 =
      typename std::conditional<SERPENT_ZMIJ_MSC_VER != 0, int16_t[8], int16x8_t>::type;
...
#if SERPENT_ZMIJ_USE_NEON  // An optimized version for NEON by Dougall Johnson.

// Converts four numbers < 10000, one in each 32-bit lane, to BCD digits.
SERPENT_ZMIJ_INLINE auto to_bcd_4x4(int32x4_t efgh_abcd_mnop_ijkl,
                            const data& d) noexcept -> uint8x16_t {
  // Compiler barrier, or clang breaks the subsequent MLA into UADDW + MUL.
  SERPENT_ZMIJ_ASM(("" : "+w"(efgh_abcd_mnop_ijkl)));
...
SERPENT_ZMIJ_INLINE auto to_bcd_4x4(__m128i y, const data& d) noexcept -> __m128i {
...
auto to_bcd8(uint64_t abcdefgh) noexcept -> bcd_result {
  if (!SERPENT_ZMIJ_USE_SSE && !SERPENT_ZMIJ_USE_NEON) {
  ...
  SERPENT_ZMIJ_ASM(("" : "+r"(d)));  // Load constants from memory.
...
SERPENT_ZMIJ_INLINE auto to_digits(uint64_t value, const data& d) noexcept
    -> dec_digits<num_bits> {
#if !SERPENT_ZMIJ_USE_NEON && !SERPENT_ZMIJ_USE_SSE
  ...
  if (!SERPENT_ZMIJ_USE_SSE4_1) y = _mm_shuffle_epi32(y, _MM_SHUFFLE(0, 1, 2, 3));
  ...
  int len = SERPENT_ZMIJ_USE_SSE4_1 ? 16 - ctz(mask) : 64 - clz(mask);
#  if SERPENT_ZMIJ_USE_SSE4_1
...
template <>
SERPENT_ZMIJ_INLINE auto to_digits<32>(uint64_t value,
                               [[SERPENT_ZMIJ_MAYBE_UNUSED]] const data& d) noexcept
    -> dec_digits<32> {
#if SERPENT_ZMIJ_USE_SSE4_1
...
SERPENT_ZMIJ_INLINE void write_digits(char* buffer, dec_digits<64>::digits_type digits,
                              bool drop_leading_zero, const data& d) noexcept {
  if (!SERPENT_ZMIJ_USE_NEON && !SERPENT_ZMIJ_USE_SSE4_1) {
...
SERPENT_ZMIJ_INLINE void write_digits(char* buffer, uint64_t digits,
...
SERPENT_ZMIJ_INLINE auto write_scientific_simd(char* buffer, const dec_digits<32>& dig,
...
SERPENT_ZMIJ_INLINE auto write_scientific_simd(char*, const dec_digits<64>&, int, bool,
...
struct writer {
  ...
  SERPENT_ZMIJ_INLINE auto write(char c) noexcept -> size_t {
    if (out < end) *out++ = c;
    return ++count;
  }
  // Copies at most `end - out` chars of `s[0..n)`; `n` must be non-negative.
  SERPENT_ZMIJ_INLINE auto write(const char* s, int n) noexcept -> size_t {
    assert(n >= 0);
  ...
  SERPENT_ZMIJ_INLINE auto write_zeros(int n) noexcept -> size_t {
    assert(n >= 0);
    size_t take = n < end - out ? size_t(n) : size_t(end - out);
    memset(out, '0', take);
    out += take;
...
SERPENT_ZMIJ_INLINE auto write_zero(char* buffer, int precision) noexcept -> char* {
  *buffer++ = '0';
...
SERPENT_ZMIJ_INLINE auto write_exp(char* buffer, int dec_exp) noexcept -> char* {
...
SERPENT_ZMIJ_INLINE void normalize(UInt& bin_sig, int64_t& bin_exp) noexcept {
...
template <typename Float>
SERPENT_ZMIJ_INLINE auto write_scientific_digits(char* buffer,
...
struct shortest_decimal {
  uint64_t sig;
  int exp;
  int last_digit SERPENT_ZMIJ_DEFAULT(0);
  bool has_last_digit SERPENT_ZMIJ_DEFAULT(false);
};
...
template <typename Float, typename UInt>
SERPENT_ZMIJ_INLINE auto to_decimal(UInt bin_sig, int64_t raw_exp, bool regular,
  ...
  if (!regular) [[SERPENT_ZMIJ_UNLIKELY]] {
  ...
  SERPENT_ZMIJ_ASM(("" : "+r"(dec_exp)));  // Force 32-bit reg for sxtw addressing.
  unsigned char shift =
  ...
  if (num_bits == 32) {
    ...
    int digit = int((fractional * 10 + (uint64_t(1) << (extra_shift - 1))) >>
                    extra_shift);
    if (fractional == (uint64_t(1) << (extra_shift - 2))) [[SERPENT_ZMIJ_UNLIKELY]]
  ...
  if (fractional == (1ull << 62)) [[SERPENT_ZMIJ_UNLIKELY]]
    digit = 2;  // Round 2.5 to 2.
  return {integral, dec_exp, digit, (round_up + round_down) == 0};
}
...
template <typename Float>
SERPENT_ZMIJ_INLINE auto scale(uint64_t bin_sig, int bin_exp, int dec_exp) noexcept
    -> scaled_value {
...
SERPENT_ZMIJ_INLINE auto round_even(scaled_value value) noexcept -> uint64_t {
...
template <typename Float>
SERPENT_ZMIJ_INLINE auto to_decimal(uint64_t bin_sig, int bin_exp,
...
auto write_number(writer& w, const char* digits, int num_digits, int lead_exp,
  ...
  if (!fixed) {
    ...
    char exp[8];
    char* end = serpent::external::zmij::detail::write_big_exp(exp, lead_exp);
...
auto write_big(writer& w, bigint num, int bin_exp, int precision, char* digits,
               int digits_size, serpent::external::zmij::format fmt) noexcept -> size_t {
  bool general = fmt == serpent::external::zmij::format::general;
  bool fixed = fmt == serpent::external::zmij::format::fixed;
...
namespace serpent::external::zmij {
...
auto to_decimal(Float value) noexcept -> dec_fp<> {
  ...
  if (bin_exp == 0 || bin_exp == traits::exp_mask) [[SERPENT_ZMIJ_UNLIKELY]] {
...
auto write(char* buffer, Float value) noexcept -> char* {
  ...
  *buffer = '-';
  ...
  if (!traits::is_normal(bin_exp)) [[SERPENT_ZMIJ_UNLIKELY]] {
  ...
  if (traits::num_bits == 32 && dec.sig < uint32_t(1e6)) [[SERPENT_ZMIJ_UNLIKELY]] {
  ...
  if (dec_exp >= traits::min_fixed_dec_exp &&
    ...
    if (SERPENT_ZMIJ_AARCH64) SERPENT_ZMIJ_ASM(("" : "+r"(fixed_layouts)));
...
auto to_decimal_big(Float value) noexcept -> dec_fp<uint128_t> {
  ...
  if (!traits::is_normal(raw_exp)) [[SERPENT_ZMIJ_UNLIKELY]] {
...
auto write_fixed(char* buffer, Float value, int precision) noexcept -> char* {
  ...
  *buffer = '-';
  ...
  if (num_digits <= 0) [[SERPENT_ZMIJ_UNLIKELY]] {
  ...
  if (num_digits > 18) [[SERPENT_ZMIJ_UNLIKELY]]
